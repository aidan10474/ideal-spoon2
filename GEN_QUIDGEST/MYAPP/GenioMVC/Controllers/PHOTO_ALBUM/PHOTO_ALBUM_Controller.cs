...
// USE /[MANUAL TRA MANUAL_CONTROLLER PHOTO_ALBUM]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			...
			try
			{
				...
				{
					case "F_PHOTO_ALBUM__PROPERTY__TITLE":	// Field (DB)
						{
							var model = new F_photo_album_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_photo_album__property__title(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_PHOTO_ALBUM__PROPERTY__TITLE":	// Field (DB)
						values = new F_photo_album_ViewModel(UserContext.Current).GetDependant_F_photo_albumTablePropertyTitle(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_photo_album" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_photo_album([FromBody]F_photo_album_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "photo_album",
				(primaryKey) => Models.Photo_album.Find(primaryKey, UserContext.Current, "FF_PHOTO_ALBUM"),
