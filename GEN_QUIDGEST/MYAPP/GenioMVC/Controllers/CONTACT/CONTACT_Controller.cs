...
// USE /[MANUAL TRA MANUAL_CONTROLLER CONTACT]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			...
			try
			{
				...
				{
					case "F_CONTACT__PROPERTY__TITLE":	// Field (DB)
						{
							var model = new F_contact_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_contact__property__title(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_CONTACT__PROPERTY__TITLE":	// Field (DB)
						values = new F_contact_ViewModel(UserContext.Current).GetDependant_F_contactTablePropertyTitle(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_contact" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_contact([FromBody]F_contact_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "contact",
				(primaryKey) => Models.Contact.Find(primaryKey, UserContext.Current, "FF_CONTACT"),
