...
// USE /[MANUAL TRA MANUAL_CONTROLLER CITY]/
		...
		public JsonResult ReloadDBEdit([FromBody]RequestReloadDBEditModel requestModel)
		{
			...
			*/
			...
			try
			{
				...
				{
					case "F_CITY__COUNTRY__COUNTRY":	// Field (DB)
						{
							var model = new F_city_ViewModel(UserContext.Current) { editable = false };
							model.MapFromModel(row);
							model.Load_F_city__country__country(qs);
		...
		public JsonResult GetDependants([FromBody]RequestDependantsModel requestModel)
		{
			...
			try
			{
				...
				{
					case "F_CITY__COUNTRY__COUNTRY":	// Field (DB)
						values = new F_city_ViewModel(UserContext.Current).GetDependant_F_cityTableCountryCountry(Selected);
						break;
					default: break;
				}
		...
		/// <summary>
		/// Recalculate formulas of the "F_city" form. (++, CT, SR, CL and U1)
		/// </summary>
		/// <param name="formData">Current form data</param>
		/// <returns></returns>
		[HttpPost]
		public JsonResult RecalculateFormulas_F_city([FromBody]F_city_ViewModel formData)
		{
			return GenericRecalculateFormulas(formData, "city",
				(primaryKey) => Models.City.Find(primaryKey, UserContext.Current, "FF_CITY"),
