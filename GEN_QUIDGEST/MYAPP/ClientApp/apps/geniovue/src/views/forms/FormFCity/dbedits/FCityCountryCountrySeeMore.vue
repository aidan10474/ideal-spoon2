<template>
	<teleport
		v-if="isReady"
		to="#q-modal-see-more-f-city-country-country-body">
		<q-row>
			<q-table
				v-bind="listCtrl"
...
<script>
	...
	const requiredTextResources = ['F_CITY__COUNTRY__COUNTRY_SeeMore', 'hardcoded', 'messages']

	export default {
		name: 'FCityCountryCountrySeeMore',
		...
		data()
		{
			return {
				...
				componentOnLoadProc: asyncProcM.getProcListMonitor('F_CITY__COUNTRY__COUNTRY_SeeMore', false),

				interfaceMetadata: {
					id: 'F_CITY__COUNTRY__COUNTRY_SeeMore', // Used for resources
					requiredTextResources
				},

				menuInfo: {
					acronym: 'F_CITY__COUNTRY__COUNTRY_SeeMore',
					name: 'F_CITY__COUNTRY__COUNTRY_SeeMore',
					controller: 'CITY',
					action: 'F_CITY_CountryValCountry'
				},
		...
		mounted()
		{
			...
			const modalProps = {
				id: 'see-more-f-city-country-country',
				dismissAction: this.close,
				returnElement: 'F_CITY__COUNTRY__COUNTRY_see-more_button'
			}
			const props = {
				class: 'q-dialog-see-more',
				title: computed(() => this.Resources.COUNTRIES03108),
				buttons: [
					{
		...
		beforeUnmount()
		{
			...
			removeModal('see-more-f-city-country-country')
		},
			...
			getListConfig()
			{
				...
				const listProps = {
					configuration: {
						...
						uuid: 'F_city_F_city_CountryValCountry',
